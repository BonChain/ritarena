...
Plays a dramatic "REKT" or "ELIMINATED" overlay when triggered. Auto-hides after 2 seconds.
...
A boolean that stays `true` across two eliminations won't re-fire (the `useEffect` dependency doesn't change). Use a counter or timestamp that increments per elimination.
