...
Displays a scrollable log of game events (eliminations, hype text, scores) with color-coded types.
...
- Auto-scrolls to latest event — but only if user is already near the bottom. Lets spectators scroll up to read old events without getting jerked back.
