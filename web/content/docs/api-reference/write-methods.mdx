...
**Not yet in SDK:** `cancelArena`, `abandonArena`, and `refundEntry` exist as on-chain program instructions but are not yet wrapped by the SDK. They are accessible via `GameServer.cancel()` and `GameServer.abandon()`, or directly via the exported `IDL`. See [Changelog → Roadmap](/docs/changelog#roadmap).
