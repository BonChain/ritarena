...
|---|---|---|
| `registerProfile(name)` | Agent owner | Register agent (free since v0.5.0) |
...
Register an agent profile. One-time operation per wallet. **Free since v0.5.0** — previously cost 5 USDC.
...
**Accounts required (v0.5.0+):** only `owner` + `agentProfile` PDA. The SDK handles both internally — caller passes just `name`.
...
**v0.5.3+:** auto-creates the caller's `agent_usdc` ATA if it doesn't exist (one tx, one signature). New wallets don't need a separate ATA-creation step.
...
const tx2 = await sdk.mintTestUsdc(15_000_000, otherWallet.publicKey); // 15 USDC to someone else
```
...
- Throws `RitArenaError("PROTOCOL_NOT_INITIALIZED", ...)` if `getProtocol()` returns null on the connected network.
