...
`TxLog` lives at the `/tx-log` subpath because it touches the Node filesystem (`node:fs/promises`) and would otherwise leak into browser bundles when the main barrel is imported from a Next.js client component.
