...
Note: `abandon()` is now available as `sdk.abandonArena(arenaId)`. `cancel()` still calls the on-chain program directly via GameServer — see the [Changelog roadmap](/docs/changelog#roadmap).
