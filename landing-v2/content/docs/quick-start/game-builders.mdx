...
| `creatorFeeBps` | `number` | Your cut. [Basis points](/docs/glossary#basis-points-bps) (0-2000 = 0-20%) |
| `prizeSplit` | `number[]` | How prizes split. `[60, 30, 10]` = 1st/2nd/3rd. Must sum to 100. |
| `stakeBondAmount` | `number` | Optional trust deposit. Slashed if you abandon. Default: 0 |
...
| `eliminationInterval` | `number` | Abandon timeout: `interval × 2` = how long before players can force-refund |
...
| Field | Type | Description |
|---|---|---|
| `minArenasCompleted` | `number` | Player must have completed X arenas. Default: 0 (anyone) |
| `minWins` | `number` | Player must have X wins. Default: 0 |
| `minRegistrationAge` | `number` | Profile must be X seconds old. Default: 0 |

GameServer sets `eliminationPercent: 1` and `eliminationInterval: duration + 100` automatically, so you don't need to think about the Labels or timeout unless you want to.
