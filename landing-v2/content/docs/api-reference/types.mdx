...
interface CreateArenaConfig {
  // ⚡ ENFORCED — program validates and uses these
  ...
  creatorFeeBps: number;         // 0-2000 (0-20%). Paid from pool on finalize.
  prizeSplit: number[];          // Must sum to 100. Max 10 slots. [60,30,10]
  stakeBondAmount?: number;      // Slashed if abandoned. Default: 0

  // 🕐 TIMEOUT — only used for abandon: timeout = eliminationInterval × 2
  eliminationInterval: number;   // Must be > 0. Set high for manual control.

  // 📋 METADATA — stored on-chain, NOT enforced during gameplay
  duration: number;              // Display hint. Must be > 0. Oracle controls actual timing.
  eliminationPercent: number;    // Display hint. 1-99. Oracle decides who dies.
  actionSchema: string;          // Valid actions hint. Max 256 chars. Server enforces.
  rulesHash: Uint8Array;         // SHA256 of rules. For verification only.

  // 🚪 ENTRY GATES — enforced on enterArena()
  minArenasCompleted?: number;   // Default: 0 (no gate)
  minWins?: number;              // Default: 0
  minRegistrationAge?: number;   // Seconds. Default: 0
}
```
