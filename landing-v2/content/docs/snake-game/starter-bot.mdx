...
Train a neural network using reinforcement learning on game replays. Export to ONNX, load in your agent, run inference per tick. The Merkle-verified game logs from past arenas are literally RL training data — that's [the flywheel](https://ritarena.com/creators).
...
Source code: [`examples/snake-game/src/agent/strategies.ts`](https://github.com/BonChain/ritarena/tree/main/examples/snake-game/src/agent/strategies.ts)
